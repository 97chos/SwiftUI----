...
- SwiftUI에서 사용하는 `@State, @Binding, @Published, @ObservedObject` 등이 Propert Wrapper에 해당됩니다. 
...
- 위 예제처럼 타입 앞에 `@propetyWrapper`를 붙여 컴파일러에게 **해당 타입은 일반 타입이 아닌 행동을 정의하는 타입**임을 알려줍니다.
...
- address 상수에 저장된 Address.town은 Uppercase라는 propertWrapper를 거쳐 동작합니다.
- 엄밀히 말하자면 town은 String 형이 아닌, PropertyWrapper의 WrappedValue로 래핑된 값의 성질을 띄게 됩니다.
- init으로 데이터의 할당과 동시에 wrappedValue에 정의된 Set을 거쳐 데이터를 처리한 다음 할당됩니다.
...
- 각각의 변수마다 get,set 부분이 중복되고 있으며, 각각의 Key와 다른 타입의 value 값을 갖고 있습니다.
...
- 이런식으로 UserDefault의 이니셜라이징을 이용하여 기본 값을 지정 할당하여 개선할 수도 있습니다. 


## @State

- 일반적으로 struct는 값 타입이어서, struct 내의 값을 변경할 수 없습니다.
- SwiftUI는 **View의 상태값을 나타내는 @State를 제공**하여, struct 내의 값을 변경할 수 있도록 도와줍니다. 
- SwiftUI의 View는 Struct로 이루어져있기 때문에, @State를 사용하여 지속적으로 변형 가능한 변수를 만들게 됩니다.
- 선언한 View 외부에서는 사용할 수 없고, private 형태로 내부에서만 참조가 가능합니다.
- 이렇게 보면 View가 직접 @State 값을 갖고 있는 것처럼 보여지나, 사실 저장되는 곳은 View 외부의 메모리에 저장됩니다.
- 외부메모리에 **@State가 변경될 때마다 변경된 값을 재참조하여 View를 다시 렌더링**합니다.
- `View 내부에서 @State 선언 -> View 외부 메모리에 @State 값 저장 -> View 내부에서 @State 변경 -> View 삭제 -> 외부 메모리에 저장된 변경된 @State 값을 참조하여 View를 다시 재생성`
